...
class MainWindow(customtkinter.CTk):
    def __init__(self):
        ...
        # Надпись База №1
        self.base1 = customtkinter.CTkLabel(self, text = 'База Поста')
        ...
        # Ввод даты
        self.label_data = customtkinter.CTkLabel(self, text = 'Выбор даты')
        self.label_data.grid(column=0, row=6, sticky='n', padx=15, pady=60, columnspan=2)
        self.entry_year = customtkinter.CTkEntry(self, width = 150)
        self.entry_year.grid(column=1, row=6, sticky='n', padx=10, pady=100)
        self.entry_month = customtkinter.CTkEntry(self, width = 150, placeholder_text='Формат: 01')
        self.entry_month.grid(column=1, row=6, sticky='n', padx=10, pady=140)
        self.entry_day = customtkinter.CTkEntry(self, width = 150, placeholder_text='Формат: 01')
        self.entry_day.grid(column=1, row=6, sticky='n', padx=10, pady=180)
        # Надписи даты
        self.entry_year_label = customtkinter.CTkLabel(self, text = 'Год:  ', justify='left')
        self.entry_year_label.grid(column=0, row=6, sticky='n', padx=10, pady=100)
        self.entry_month_label = customtkinter.CTkLabel(self, text = 'Месяц:  ', justify='left')
        self.entry_month_label.grid(column=0, row=6, sticky='n', padx=10, pady=140)
        self.entry_day_label = customtkinter.CTkLabel(self, text = 'День:  ', justify='left')
        self.entry_day_label.grid(column=0, row=6, sticky='n', padx=10, pady=180)
        
        
       
        # # Ввод данных blacklist
        # Надпись База №1
        self.base2 = customtkinter.CTkLabel(self, text = 'База Чёрного Списка')
        ...
        self.btn_monitoring.grid(column=0, row=6, sticky='n', padx=10, pady=275, columnspan=4)

        self.btn_exit = customtkinter.CTkButton(self, text = 'Выход', command = self.commandexit)
        self.btn_exit.grid(column=3, row=6, sticky='n', padx=10, pady=325)
        
        self.btn_data = customtkinter.CTkButton(self, text = 'Внести дату', command = self.insert_data)
        self.btn_data.grid(column=0, row=6, sticky='n', padx=10, pady=230, columnspan=2)
    ...
    def connect2(self):
        try:
            ...
            messagebox.showerror("Ошибка", "Вторая база недоступна!")
    ...
    def insert_data(self):
        try:
            self.query_data = '{}-{}-{}'.format(self.entry_year.get(), self.entry_month.get(), self.entry_day.get())
            messagebox.showinfo(title = 'Сообщение', message= 'Дата внесена')
        except:
            messagebox.showerror("Ошибка", "Дата не внесена")


class Monitoring(customtkinter.CTk):
    def __init__(self, cur1, cur2, conn1, conn2, query_data):
        super().__init__()
        self.cur1 = cur1
        self.cur2 = cur2
        self.conn1 = conn1
        ...
        self.btn_exit = customtkinter.CTkButton(self, text = 'Выход', command = self.commandexit)
        self.btn_exit.grid(column=1, row=3, sticky='s', padx=10, pady=10)
        self.btn_exportxlcx = customtkinter.CTkButton(self, text = 'Сохранить в CSV',  command = self.write_to_excel, fg_color = 'green')
        self.btn_exportxlcx.grid(column = 0, row = 2, sticky = 's', padx = 10, pady = 10)
        self.f = 0

    def query(self):
        while True:
            try:
                self.cur1.execute("SELECT s_talker FROM sprut.spr_speech_table WHERE s_datetime > '{}'".format(self.query_data))
    ...
    def write_to_excel(self):
        try:
            list_of_dfs = []
            for item in self.common:
                query_final = "SELECT s_datetime, s_sysnumber, s_talker, s_basestation, s_lac, s_cid FROM sprut.spr_speech_table WHERE s_talker='{}'".format(item)
                df = pd.read_sql_query(query_final, self.conn1)
                list_of_dfs.append(df)
            merged_dfs = pd.concat(list_of_dfs, axis=0)
            merged_dfs.to_csv("{}.csv".format(str(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))), index=False)
        except:
            messagebox.showerror("Ошибка", "Нету совбадений")
    
def main():
    mainwindow = MainWindow()
    mainwindow.mainloop()
    if mainwindow.conn1 and mainwindow.conn2:
        conn1 = mainwindow.conn1
        conn2 = mainwindow.conn2
        cur1 = mainwindow.cur1
        cur2 = mainwindow.cur2
        query_data = mainwindow.query_data
        monitoring_window = Monitoring(cur1, cur2, conn1, conn2, query_data)
