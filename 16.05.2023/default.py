...
class Monitoring(customtkinter.CTk):
    def __init__(self, cur1, cur2, conn1, conn2):
        ...
        self.btn_exportxlcx = customtkinter.CTkButton(self, text = 'Сохранить в Excel',  command = self.write_to_excel)
        self.btn_exportxlcx.grid(column = 0, row = 3, sticky = 's', padx = 12, pady = 12)
        self.f = 0

    def query(self):
        while True:
            try:
                self.cur1.execute("SELECT s_talker FROM sprut.spr_speech_table")
    ...
    def write_to_excel(self):
        try:
            list_of_dfs = []
            for item in self.common:
                zapros = "SELECT s_datetime, s_sysnumber, s_talker, s_basestation, s_lac, s_cid FROM sprut.spr_speech_table WHERE s_talker='{}'".format(item)
                df = pd.read_sql_query(zapros, self.conn1)
